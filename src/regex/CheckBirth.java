...
public class CheckBirth {
    ...
    public static boolean isDate(String validate) {
        ...
        if (!isDateMin(day, month, year)) {
            System.err.println("Birth min 11 year");
            return false;
        }
        if (resultDay == 0) {
            return false;
    ...
    private static boolean isDateMin(int day, int month, int year) {
        LocalDate myObj = LocalDate.now();
        String[] dateNow = myObj.toString().split("-");
        int yearNow = Integer.valueOf(dateNow[0])-11;
        if (year > yearNow) {
            return false;
        }
